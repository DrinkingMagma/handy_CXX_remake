...
namespace handy
{
    ...
    /**
     * @brief 匿名命名空间，用于封装信号处理相关的全局变量和函数，避免外部链接冲突
     * @details 该命名空间包含信号处理函数的映射表及其保护互斥锁，确保多线程环境下信号处理函数的安全注册、删除和调用。
     * 由于匿名命名空间的特性，其内部成员仅在当前编译单元（.cpp文件）中可见，有效防止了与其他文件的命名冲突，同时保持了代码的模块化和封装性。
    */
    namespace
    {
        /// 存储信号处理函数的映射表
